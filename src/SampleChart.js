...
class SampleChart extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.coefficient = 1.005;
    this.chartRef = React.createRef();

    this.state = {
      inputValue: this.coefficient
    }
    ...
    let _25_49data = ['25-49', 76.8, 75.0, 75.9, 76.5, 78.3, 78.1, 79.4, 84.4, 84.7, 84.3, 76.6, 74.8, 78.4, 79.7, 80.5, 80.9, 82.8, 82.1, 83.8, 83.9];
    ...
    while (i < 22) {
      ...
      _18_24data.push(this.calcNumber(this.state.inputValue,_18_24data));
      _25_49data.push(this.calcNumber(this.state.inputValue,_25_49data));
      _50_74data.push(this.calcNumber(this.state.inputValue,_50_74data));
      i++;
  ...
  render() {
    return (
      <div className="chart">
        <ReactEcharts ref={(e) => { this.chartRef.current = e; }} option={this.opts} style={{height: "70vh"}} onEvents={this.onEvents}/>
        <label for="coef">Pane mingi nr</label>
        <input step="0.005" type="number" value={this.state.inputValue} id="coef" onChange={(evt) => this.setState({inputValue: evt.target.value})} />
      </div>
    )
  }
};
